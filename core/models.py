...
class CalendarEvent(Base):
    """Google_id artık kullanıcı başına benzersiz (tekil değil) — Google hesabı paylaşılan tek
    hesap olduğu için aynı dış etkinlik teoride birden fazla kullanıcının senkronuna girebilir."""

    __tablename__ = "calendar_events"
    __table_args__ = (UniqueConstraint("user_id", "google_id", name="uq_calendar_events_user_google"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    google_id: Mapped[str] = mapped_column(String(300), nullable=False)
